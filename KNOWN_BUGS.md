...
**2. Supabase project auto-pauses after ~7 days idle.**
- Free-tier Supabase projects pause themselves after about a week with no traffic.
- The first query after a pause can fail or time out until the project wakes back up.
- Same category as #1 above - noticed after a week-long gap in work caused a transient `/login` failure (see the split 401/500 error handling this added).

**3. MongoDB Atlas network access is open (`0.0.0.0/0`).**
...
**4. Geo-fencing uses straight-line distance, not travel distance.**
...
**5. Cycling routes depend on the public OSRM server.**
...
**6. Seeded-device coastline check is an approximation.**
...
**7. Registration management is delete-only.**
...
**10. Automated tests are small, not comprehensive.**
...
**11. Map rendering verified in-browser only.**
